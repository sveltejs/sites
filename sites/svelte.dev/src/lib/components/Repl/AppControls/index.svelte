<script>
	...
	import { download_blob } from '$lib/utils.js';
	import { enter_key } from '$lib/utils';
	...
	function handleKeydown(event) {
		if (event.key === 's' && (is_mac ? event.metaKey : event.ctrlKey)) {
...
<div class="app-controls">
	<input bind:value={name} on:focus={e => e.target.select()} use:enter_key={e => e.target.blur()} />
